...
const Sidebar = () => {
  ...
  // Logic to match the Login Page Button Gradient
  ...
  return (
    <>
      {/* 1. Mobile Top Bar */}
      <div className="flex w-full items-center justify-between bg-[#05010a] px-6 py-4 text-white md:hidden border-b border-white/5">
        <button onClick={() => setIsOpen(true)}>
          <HiMenuAlt2 className="text-2xl text-purple-500" />
        </button>
        <span className="font-black uppercase tracking-tighter text-lg">AI <span className="text-purple-500">CHAT</span></span>
        <div className="h-9 w-9 rounded-full bg-gradient-to-r from-[#7b2ff7] to-[#4b10b0] p-[2px]">
           <img src={user?.image} className="h-full w-full rounded-full bg-[#05010a] object-cover" alt="profile" />
        </div>
      </div>
      ...
      `}>
        ...
        {/* Decorative Ambient Glow (Matches Login Page) */}
        <div className="absolute top-0 left-0 w-full h-64 bg-[radial-gradient(circle_at_top_left,_rgba(123,47,247,0.1),_transparent_70%)] pointer-events-none"></div>

        {/* Logo Section */}
        <div className="px-8 py-12 flex items-center gap-4 relative z-10">
          <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-br from-[#7b2ff7] to-[#4b10b0] text-white shadow-[0_0_20px_rgba(123,47,247,0.5)]">
            <span className="text-2xl font-black">C</span>
          </div>
          <div className="flex flex-col">
            <h1 className="text-xl font-black text-white tracking-tighter leading-none uppercase">AI CHAT</h1>
            <span className="text-[10px] font-bold text-purple-500 tracking-[0.3em] uppercase">Premium</span>
          </div>
        </div>
        ...
        <div className="mt-auto p-6 space-y-6 relative z-10">
          ...
          {/* User Card - Glassmorphism style from Login */}
